...
#define REPORT_FIRST_ACCEL_CAP 20u
#define REPORT_NEXT_ACCEL_CAP  22u
...
static void protocol_send_frame(const char *frame, uint16_t frame_len);
...
static void protocol_send_frame(const char *frame, uint16_t frame_len)
{
    ...
    while (sent < frame_len)
    {
        ...
        if (rc != CDC_SUCCESS)
        {
            LOG_PRINT("protocol tx busy/not-ready rc=%d\r\n", (int)rc);
            break;
        }
...
static void protocol_process_one_frame(const char *frame, uint16_t frame_len)
{
    ...
    if (type == MSG_SET_PARAMS)
    {
        ...
        if (ack_len > 0u)
        {
            protocol_send_frame(ack, ack_len);
