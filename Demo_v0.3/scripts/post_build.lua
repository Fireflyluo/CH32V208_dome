...
local function parse_memory_from_ldscript(ldscript_path)
    ...
    if not os.isfile(ldscript_path) then
        return rom_total, ram_total, flash0_total, flash1_total
    end
...
function main(target, toolchain_path, cross_prefix)
    ...
    local rom_total, ram_total, flash0_total, flash1_total
    for _, ld_path in ipairs(ldscript_paths) do
        rom_total, ram_total, flash0_total, flash1_total = parse_memory_from_ldscript(ld_path)
    ...
    local function show_memory_usage_eide()
        ...
        local flash1_used = clamp0(flash1_text_size + flash1_rodata_size)
