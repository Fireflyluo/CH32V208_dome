...
#define OLED_I2C_DEV_ADDR 0x3C                        // OLED设备I2C地址(0x3C)
#define OLED_I2C_OWNER_ID 3U                          // OLED设备所有者ID
#define OLED_DMA_BULK_THRESHOLD 16U                   // DMA传输数据块大小阈值
#define OLED_I2C_REQ_TIMEOUT_MS 50U                   // I2C总线请求超时时间
#define OLED_I2C_FRAME_TIMEOUT_MS 1500U               /* 帧刷新超时 */
#define OLED_WIDTH 128U                               // OLED宽度
#define OLED_HEIGHT 64U                               // OLED高度
#define OLED_PAGE_CNT (OLED_HEIGHT / 8U)              // OLED页数
#define OLED_FRAME_BYTES (OLED_WIDTH * OLED_PAGE_CNT) // OLED帧字节数
...
/**
 * OLED命令提交函数
 * 通过I2C总线向OLED设备发送控制命令
 * @param cmds 命令数据指针
 * @param len 命令数据长度
 * @return 0 成功，-1 参数错误，其他值 I2C通信错误
 */
static int oled_submit_commands(const uint8_t *cmds, uint16_t len)
{
    ...
    // 配置I2C总线请求参数
    req.bus = I2C_NUM_1;                      // 使用I2C1总线
    req.type = I2C_BUS_REQ_WRITE_REG;         // 写寄存器类型请求
    req.owner_id = OLED_I2C_OWNER_ID;         // OLED设备所有者ID，用于总线仲裁
    req.dev_addr = OLED_I2C_DEV_ADDR;         // OLED设备I2C地址(0x3C)
    req.reg = 0x00;                           // 寄存器地址为0x00，表示命令模式
    req.wbuf = cmds;                          // 写入的数据缓冲区
    req.rbuf = NULL;                          // 无需读取数据
    req.len = len;                            // 数据长度
    req.mode_hint = I2C_MODE_IT;              // 使用中断模式传输
    req.prio = I2C_BUS_PRIO_LOW;              // 低优先级请求
    req.timeout_ms = OLED_I2C_REQ_TIMEOUT_MS; // 超时时间50ms
...
static int oled_reassert_horizontal_state(void)
{
    ...
    static const uint8_t cmds[] = {
        0x20, 0x00, /* Horizontal Addressing Mode - 设置为水平寻址模式 */
        0x2E,       /* Deactivate scroll - 关闭滚动功能 */
        0xD3, 0x00, /* Display offset = 0 - 显示偏移量设为0 */
        0x40        /* Display start line = 0 - 显示起始行为0 */
    };
...
/**
 * 设置OLED显示光标位置（带参数检查）
 * OLED显示屏按页(Page)组织，每页8个像素高
 * @param Page 页号，范围0-7（对应Y坐标0-63）
...
static int oled_set_cursor_checked(uint8_t Page, uint8_t X)
{
    ...
    // 构造设置光标的命令序列
    cmds[0] = (uint8_t)(0xB0U | Page);               // 设置页地址命令(0xB0 + Page)
    cmds[1] = (uint8_t)(0x10U | ((X & 0xF0U) >> 4)); // 设置列地址高4位
    cmds[2] = (uint8_t)(0x00U | (X & 0x0FU));        // 设置列地址低4位
...
static int oled_write_data_checked(uint8_t *Data, uint16_t Count)
{
    ...
    while (offset < Count)
    {
        ...
        // 等待I2C总线空闲，防止连续操作导致总线冲突
        uint32_t i2c_idle_timeout = I2C_TIMEOUT_DEFAULT;
        while (I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY) != RESET)
        {
            if (i2c_idle_timeout-- == 0U)
            {
                return -2;
            }
        }
...
/**
 * 刷新OLED指定区域的显示内容
 * 根据OLED的寻址模式选择不同的刷新策略
 * @param x 起始X坐标
 * @param page_start 起始页号
 * @param page_end 结束页号
 * @param width 区域宽度
 */
...
static void oled_refresh_area_pages(uint8_t x, uint8_t page_start, uint8_t page_end, uint8_t width)
{
    ...
    // 设置显示窗口范围
    cmd_window[0] = 0x21;                      // 列地址设置命令
    cmd_window[1] = x;                         // 起始列地址
    cmd_window[2] = (uint8_t)(x + width - 1U); // 结束列地址
    cmd_window[3] = 0x22;                      // 页地址设置命令
    cmd_window[4] = page_start;                // 起始页地址
    cmd_window[5] = page_end;                  // 结束页地址
