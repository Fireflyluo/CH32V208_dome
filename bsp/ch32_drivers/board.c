...
#include "sht40_hal.h"
#include "usb_lib.h"
static void accel_read_callback(i2c_num_t i2c_num, int result)
{
    // 读操作完成处理，解析数据
}
static void accel_write_callback(i2c_num_t i2c_num, int result)
{
    // 写操作完成处理
}
...
static sc7a20_status_t i2c_write_reg(uint8_t reg, const uint8_t *data, uint16_t len)
{
    ...
    i2c_write_register_async(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len, accel_write_callback);
    return SC7A20_OK;
}
...
static sc7a20_status_t i2c_read_reg(uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    i2c_read_register_async(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len, accel_read_callback);
    return SC7A20_OK;
}
