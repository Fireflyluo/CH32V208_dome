...
7. `protocol_task_init()`
8. `while(1) { TMOS_SystemProcess(); }`
...
- OLED 刷新周期：**200 ms（5 Hz）**
- 字号：**`OLED_6X8` 小字**
- 显示内容（8 行）：
  1. `RDY/AHz/SHz`：传感器就绪与采样统计
  2. `A:x,y,z`：三轴加速度（mg）一行显示
  3. `T/H`：温度与湿度
  4. `TX/SQ`：当前发送包类型与序号
  5. `UP`：上传状态、分片进度、缓存样本数
  6. `TH/A12`：阈值范围与当前加速度 12bit 值
  7. `N/M/I/H`：各类上报帧累计计数
  8. `Q/T2/OT`：参数确认计数、采样周期(ms)、超阈值标志
