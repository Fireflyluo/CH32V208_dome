...
static void adhoc_sm_release_upstream_binding(adhoc_sm_t *sm, uint8_t gateway_no, uint8_t upstream_level, uint32_t upstream_id)
{
    ...
    for (index = 0u; index < ADHOC_SM_UPSTREAM_BINDING_CAPACITY; ++index)
    {
        ...
        if (binding->gateway_no == gateway_no &&
            binding->upstream_level == upstream_level &&
...
static uint8_t adhoc_sm_pick_upstream_no(adhoc_sm_t *sm, uint8_t gateway_no, uint8_t upstream_level, uint32_t upstream_id)
{
    ...
    uint8_t used_no_mask = 0u;
    adhoc_sm_upstream_binding_t *binding;

    if (sm == 0 || upstream_id == 0u || gateway_no > 7u || upstream_level > ADHOC_SM_JOIN_LEVEL_MAX)
    {
        return 0u;
    }
...
{
    if (rssi >= ADHOC_SM_RSSI_STRONG_MIN_DBM)
    {
        return 3u;
    }
    if (rssi >= ADHOC_SM_RSSI_MEDIUM_MIN_DBM)
    {
        return 2u;
    }
    return 1u;
...
{
    if (left_signal_rank != right_signal_rank)
    {
        return left_signal_rank > right_signal_rank ? 1 : -1;
    }
    if (left_rssi != right_rssi)
    {
        return left_rssi > right_rssi ? 1 : -1;
    }
    if (left_upstream_level != right_upstream_level)
    {
        return left_upstream_level < right_upstream_level ? 1 : -1;
    }
    if (left_gateway_no != right_gateway_no)
    {
        return left_gateway_no < right_gateway_no ? 1 : -1;
    }
    if (left_upstream_id != right_upstream_id)
    {
        return left_upstream_id < right_upstream_id ? 1 : -1;
    }
    return 0;
}
...
{
    if (required_hits == 0 || window_cycles == 0)
    {
        return;
    }
    if (signal_rank >= 3u)
...
{
    if (candidate == 0 || candidate->active == 0u)
    {
        return 0;
    }
    return candidate->gateway_no == gateway_no &&
...
{
    uint8_t required_hits = 0u;
    uint8_t window_cycles = 0u;
                                     ...
                                     uint32_t ts_us, uint32_t cycle_idx)
{
    ...
    if (candidate->has_last_hit_cycle == 0u || cycle_idx != candidate->last_hit_cycle_idx)
    {
        ...
        if (candidate->success_hits < 0xFFu)
        {
            candidate->success_hits++;
        }
    }
}
...
static int adhoc_sm_candidate_is_expired(const adhoc_sm_candidate_t *candidate, uint32_t cycle_idx, uint32_t now_us)
{
    ...
    if (candidate->network_end_us != 0u && adhoc_sm_time_reached(now_us, candidate->network_end_us))
    {
        return 1;
    }
    if (cycle_idx < candidate->first_cycle_idx)
    {
        return 1;
    }

    elapsed_cycles = cycle_idx - candidate->first_cycle_idx + 1u;
    if (candidate->success_hits < candidate->required_hits && elapsed_cycles > candidate->window_cycles)
    {
        return 1;
    }

    if (candidate->has_last_hit_cycle == 0u)
    {
        return 0;
    }
    if (cycle_idx < candidate->last_hit_cycle_idx)
    {
        return 1;
    }
...
{
    if (adhoc_sm_candidate_is_expired(candidate, cycle_idx, now_us))
    {
        return 0;
    }
...
{
    if (sm == 0 || sm->gateway_rx_window_open == 0u)
    {
        return 0;
    }
...
{
    if (current == 0 || current->active == 0u)
    {
        return 1;
    }
    ...
    for (index = 0u; index < ADHOC_SM_NEIGHBOR_CACHE_CAPACITY; ++index)
    {
        ...
        if (worst_index < 0 ||
            adhoc_sm_candidate_cmp_key(neighbor->signal_rank, neighbor->last_rssi,
                                       ...
                                       sm->neighbor_cache[worst_index].signal_rank, sm->neighbor_cache[worst_index].last_rssi,
                                       sm->neighbor_cache[worst_index].upstream_level, sm->neighbor_cache[worst_index].gateway_no,
...
static int adhoc_sm_neighbor_record(adhoc_sm_t *sm, const adhoc_sm_rx_event_t *event, uint8_t signal_rank)
{
    ...
    if (slot < 0)
    {
        ...
        adhoc_sm_candidate_init(sm, &sm->neighbor_cache[slot], signal_rank, event->rssi,
                                event->gateway_no, event->level, event->sender.node_id,
                                event->ts_us, cycle_idx);
    }
...
static int adhoc_sm_select_best_ready_neighbor(adhoc_sm_t *sm, uint32_t ts_us)
{
    ...
    {
        neighbor = &sm->neighbor_cache[index];
        if (!adhoc_sm_candidate_is_ready(neighbor, cycle_idx, ts_us))
        {
            continue;
        }
        if (found == 0u ||
            adhoc_sm_is_better_candidate(&sm->candidate, neighbor->signal_rank, neighbor->last_rssi,
                                         neighbor->upstream_level, neighbor->gateway_no, neighbor->upstream_id))
        {
...
{
    if (sender_id == 0u || sender_id > ADHOC_SENDER_NODE_MAX)
    {
        return 0;
    }
    if (upstream_level == 0u)
    {
        return sender_id <= ADHOC_SM_GATEWAY_ID_MAX ? 1 : 0;
    }
...
{
    if (sm == 0 || event == 0 || sm->joined_level == 0u)
    {
        return 0;
    }
    if (event->msg_class != ADHOC_MSG_CLASS_A)
    {
        return 0;
    }
    if (event->gateway_no != sm->upstream_gateway_no)
    {
        return 0;
    }
    if (event->sender.node_id != sm->upstream_id)
    {
        return 0;
    }
...
static void adhoc_sm_enter_st1(adhoc_sm_t *sm)
{
    ...
    sm->retry_count = 0u;
    sm->next_tx_us = 0u;
    sm->upstream_last_seen_us = 0u;
    sm->gateway_network_started = 0u;
    sm->gateway_network_locked = 0u;
    sm->gateway_network_start_us = 0u;
    sm->gateway_network_end_us = 0u;
...
{
    if (sm == 0 || sm->candidate.active == 0u || joined_level == 0u)
    {
        return;
    }
...
int adhoc_sm_init(adhoc_sm_t *sm, const adhoc_sm_cfg_t *cfg)
{
    ...
    if (cfg->gateway_no > 7u || cfg->t2_us == 0u || cfg->t5_us == 0u || cfg->t2_us >= cfg->t5_us)
    {
        return 0;
    }
    if (cfg->role != ADHOC_SM_ROLE_BEACON && cfg->role != ADHOC_SM_ROLE_GATEWAY)
    {
        return 0;
    }
...
{
    if (sm == 0 || sm->inited == 0u)
    {
        return 0;
    }
    if (role != ADHOC_SM_ROLE_BEACON && role != ADHOC_SM_ROLE_GATEWAY)
    {
        return 0;
    }
...
adhoc_sm_rx_result_t adhoc_sm_on_rx(adhoc_sm_t *sm, const adhoc_sm_rx_event_t *event)
{
    ...
    if (event->sender.domain_id != sm->cfg.domain_id)
    {
        return ADHOC_SM_RX_IGNORED;
    }
    if (sm->role == ADHOC_SM_ROLE_GATEWAY)
    {
        if (!adhoc_sm_gateway_in_rx_window(sm, event->ts_us))
        {
            return ADHOC_SM_RX_IGNORED;
        }
        if (event->msg_class != ADHOC_MSG_CLASS_A || event->level == 0u)
        {
            return ADHOC_SM_RX_IGNORED;
        }
        if (!adhoc_sm_sender_matches_level(event->level, event->sender.node_id))
        {
            return ADHOC_SM_RX_IGNORED;
        }
    ...
    if (event->msg_class != ADHOC_MSG_CLASS_A || event->level >= ADHOC_SM_JOIN_LEVEL_MAX)
    {
        return ADHOC_SM_RX_IGNORED;
    }
    if (!adhoc_sm_sender_matches_level(event->level, event->sender.node_id))
    ...
    if (!adhoc_sm_neighbor_record(sm, event, signal_rank))
    {
        return ADHOC_SM_RX_IGNORED;
    }

    if (!adhoc_sm_select_best_ready_neighbor(sm, event->ts_us))
    {
        return ADHOC_SM_RX_ACCEPTED;
    }
...
int adhoc_sm_poll(adhoc_sm_t *sm, uint32_t now_us, adhoc_frame_fields_t *out_fields, uint8_t *out_has_tx)
{
    uint32_t cycle_idx;
    uint8_t join_level;
    uint32_t upstream_loss_timeout_us;

    if (sm == 0 || out_has_tx == 0 || sm->inited == 0u)
    {
        return 0;
    }
    *out_has_tx = 0u;
    ...
    if (adhoc_sm_is_unconfirmed_state(sm->state))
    {
        ...
        if (sm->retry_count >= sm->cfg.retry_max)
        {
            adhoc_sm_enter_st1(sm);
            return 1;
        }
    ...
    if (adhoc_sm_is_confirmed_state(sm->state))
    {
        ...
        if (sm->regroup_timer_active != 0u &&
            sm->cfg.regroup_interval_us != 0u &&
