...
#include "sc7a20_reg.h"
    ...
    struct sc7a20_dev_t
    {
        ...
        // 异步操作上下文（新增）
        struct {
            sc7a20_async_state_t state;     // 当前异步状态
            uint8_t reg_addr;              // 寄存器地址
            const uint8_t *write_data;     // 写入数据指针
            uint8_t *read_data;            // 读取数据指针
            uint16_t data_len;             // 数据长度
            uint16_t processed_bytes;       // 已处理字节数
            bool is_read_operation;        // 是否为读操作
            void (*callback)(sc7a20_handle_t handle, sc7a20_status_t status); // 完成回调
            uint32_t start_time;           // 操作开始时间（用于超时检测）
