...
static tmosEvents serial_upload_task_process_event(tmosTaskID task_id, tmosEvents events)
{
    ...
    if (events & SERIAL_EVT_UPLOAD)
    {
        ...
        if (s_stat_div_cnt >= SERIAL_STAT_DIV)
        {
            ...
            LOG_PRINT("rf stat tg_id=%u init=%u sync=%u tx=%lu rx=%lu rssi=%d tc=%u\r\n",
                      (unsigned int)rf_st.tg_id,
                      (unsigned int)rf_st.rf_inited,
                      (unsigned int)rf_st.synced,
                      (unsigned long)rf_st.tx_cnt,
                      (unsigned long)rf_st.rx_cnt,
                      (int)rf_st.last_rssi,
                      (unsigned int)rf_st.last_rx_tc);
