...
/**
 * @enum impact_disp_status_t
 * @brief 冲击位移算法返回状态枚举
 */
typedef enum {
  IMPACT_DISP_OK = 0,           /**< 操作成功 */
  IMPACT_DISP_ERR_ARG = -1,     /**< 参数错误 */
  IMPACT_DISP_ERR_STATE = -2,   /**< 状态错误 */
  IMPACT_DISP_ERR_NOT_READY = -3/**< 操作未就绪 */
} impact_disp_status_t;

/**
 * @enum impact_disp_state_t
 * @brief 冲击位移算法状态枚举
 */
typedef enum {
  IMPACT_DISP_STATE_UNINIT = 0,     /**< 未初始化 */
  IMPACT_DISP_STATE_IDLE = 1,       /**< 空闲 */
  IMPACT_DISP_STATE_COLLECTING = 2, /**< 数据收集中 */
  IMPACT_DISP_STATE_FINISHED = 3,   /**< 完成 */
  IMPACT_DISP_STATE_ERROR = 4       /**< 错误 */
} impact_disp_state_t;
 ...
 */
enum {
  IMPACT_DISP_QF_NONE = 0u,                             /**< 无质量问题 */
  IMPACT_DISP_QF_TS_NON_MONOTONIC = (1u << 0),          /**< 时间戳非单调递增 */
  IMPACT_DISP_QF_DT_GAP = (1u << 1),                    /**< 时间间隔过大 */
  IMPACT_DISP_QF_CLIPPED = (1u << 2),                   /**< 数据被截断 */
  IMPACT_DISP_QF_NO_RELEASE = (1u << 3),                /**< 未检测到释放 */
  IMPACT_DISP_QF_ROTATION_HIGH = (1u << 4),             /**< 旋转角度过大 */
  IMPACT_DISP_QF_LOW_SAMPLE_COUNT = (1u << 5),          /**< 采样点数太少 */
  IMPACT_DISP_QF_DURATION_TOO_SHORT = (1u << 6)         /**< 事件持续时间太短 */
};
 ...
 */
enum {
  IMPACT_DISP_SAMPLE_CLIPPED = (1u << 0)                /**< 采样数据被截断 */
};

/**
 * @struct impact_disp_cfg_t
 * @brief 冲击位移算法配置结构体
 */
typedef struct {
...
/**
 * @struct impact_disp_ctx_t
 * @brief 冲击位移算法上下文结构体
 */
typedef struct {
  impact_disp_cfg_t cfg;              /**< 配置参数 */
  impact_disp_state_t state;          /**< 当前状态 */
  uint32_t event_id;                  /**< 事件ID */
  uint32_t quality_flags;             /**< 质量标志 */
