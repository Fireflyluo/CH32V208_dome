...
def safe_stdout_write(text):
    """Write text to stdout and avoid UnicodeEncodeError on non-UTF8 consoles."""
    try:
        sys.stdout.write(text)
    except UnicodeEncodeError:
        stdout_encoding = sys.stdout.encoding or 'utf-8'
        escaped = text.encode(stdout_encoding, errors='backslashreplace').decode(
...
if __name__ == "__main__":
    ...
    if args.once:
        once_text = read_once(args.port, args.baud, encoding=args.encoding)
        safe_stdout_write(once_text)
        if once_text and not once_text.endswith('\n'):
            safe_stdout_write('\n')
    else:
