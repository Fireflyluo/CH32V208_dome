...
uint32_t HAL_GetTick(void) { return sys_tick_counter; }
// 初始化SysTick定时器
...
vu8 tx_flag = 0;
void i2c_master_tx_cplt_callback(i2c_num_t i2c_num) { tx_flag = 0; }
...
{
    // 实际实现I2C写操作
    // I2C_Master_Transmit_Polling(0x19 << 1, reg, (uint8_t *)data, len); // 示例
    // i2c_write_register(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len); // 阻塞
    tx_flag = 1;
    i2c_write_register_async(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len); // 异步
    while (tx_flag == 1)
...
{
    // 实际实现I2C读操作
    // I2C_Master_Receive_Polling(0x19 << 1, reg, (uint8_t *)data, len); // 示例
    // i2c_read_register(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len); // 阻塞
    i2c_read_register_async(I2C_NUM_1, 0x19, reg, (uint8_t *)data, len); // 异步
    return SC7A20_OK;
}
