 ...
 * Ad-Hoc-lib 多节点仿真主程序
 *
 * 功能: 在 Windows 上用多线程模拟嵌入式自组网协议运行,
 *       通过拓扑矩阵控制节点间可达性和 RSSI, 验证协议实现。
 *
 * 架构:
 *   主线程: 定时打印全网状态, 可注入故障(断开/恢复链路)
 *   N 个节点线程: 各自运行 adhoc_node_poll/fetch_tx/on_rx 循环
 *   空域信道(channel): 共享的帧广播介质, 受 CriticalSection 保护
 *
 * 场景切换: 修改 SCENARIO_MODE 宏 (0=全连通, 1=树形网状)
 *   新增场景: 在 sim_init_scenario() 中添加 #elif 分支
 *
 * 编译: gcc -std=gnu11 或 MSVC cl
 * 运行: adhoc_sim.exe  (日志输出到 logs/ 目录)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
typedef struct
{
    ...
    uint8_t        node_idx;
    uint8_t        log_slot;
    uint8_t        node_mem[4096u];
    adhoc_link_win32_ctx_t link_ctx;
    HANDLE         thread;
    volatile LONG  running;
    volatile LONG  stop;
    uint32_t       last_state;
    uint32_t       last_data_submit_us;
    uint32_t       data_seq;
    uint8_t        network_locked_logged;
} sim_node_t;
...
typedef struct
{
    ...
    uint8_t              gw_count;
    uint8_t              bcn_count;
    int8_t               topo[SIM_MAX_NODES][SIM_MAX_NODES];
    volatile LONG        stop;
} sim_ctx_t;
...
static void sim_init_scenario(void)
{
...

    {
        ...
        exe_dir[len] = '\0';
        if (strlen(exe_dir) + strlen("\\logs") >= sizeof(logs_path))
        {
            printf("[FATAL] logs path too long\n");
            exit(1);
        }
