...
static float impact_disp_rotation_limit_mg(float limit_deg) {
  ...
  return 2000.0f * sinf(half_rad);                 // 计算阈值
}
...
static int impact_disp_confidence_from_flags(uint32_t flags, uint32_t samples,
  ...
  if ((flags & IMPACT_DISP_QF_ROTATION_HIGH) != 0u) {  // 如果旋转角度过大
    score -= 35;
  }
  if ((flags & IMPACT_DISP_QF_DT_GAP) != 0u) {  // 如果时间间隔过大
    score -= 10;
  }
  if ((flags & IMPACT_DISP_QF_CLIPPED) != 0u) {  // 如果数据被截断
    score -= 10;
  }
  if ((flags & IMPACT_DISP_QF_TS_NON_MONOTONIC) != 0u) {  // 如果时间戳非单调
    score -= 20;
  }
  if ((flags & IMPACT_DISP_QF_DURATION_TOO_SHORT) != 0u) {  // 如果持续时间太短
    score -= 20;
  }
...
impact_disp_status_t impact_disp_reset(impact_disp_ctx_t *ctx) {
  impact_disp_cfg_t backup_cfg;  // 备份配置
  ...
  ctx->state = IMPACT_DISP_STATE_IDLE;  // 设置状态为空闲
  return IMPACT_DISP_OK;  // 返回成功
}
...
impact_disp_status_t impact_disp_begin_event(impact_disp_ctx_t *ctx,
  ...
  if (ctx->state == IMPACT_DISP_STATE_UNINIT) {  // 检查状态是否已初始化
  ...
  ctx->state = IMPACT_DISP_STATE_COLLECTING;  // 设置状态为数据收集
  return IMPACT_DISP_OK;  // 返回成功
}
...
impact_disp_status_t impact_disp_set_baseline_mg(impact_disp_ctx_t *ctx,
  ...
  if (ctx->state == IMPACT_DISP_STATE_UNINIT) {  // 检查是否已初始化
    return IMPACT_DISP_ERR_STATE;
  }
...
impact_disp_status_t impact_disp_feed_sample(impact_disp_ctx_t *ctx,
  ...
  if (ctx == NULL || sample == NULL) {
    return IMPACT_DISP_ERR_ARG;  // 如果上下文或采样为空，返回参数错误
  }
  if (ctx->state != IMPACT_DISP_STATE_COLLECTING) {  // 检查是否处于数据收集状态
    return IMPACT_DISP_ERR_STATE;
  }
  ...
  a_mag_dyn_mg = impact_disp_norm3(ax_dyn_mg, ay_dyn_mg, az_dyn_mg);  // 计算幅值

  if (a_mag_dyn_mg > ctx->peak_acc_mg) {  // 更新峰值加速度
    ctx->peak_acc_mg = a_mag_dyn_mg;
  }

  if ((sample->flags & IMPACT_DISP_SAMPLE_CLIPPED) != 0u) {  // 检查采样是否被截断
    ctx->quality_flags |= IMPACT_DISP_QF_CLIPPED;  // 设置质量标志
  }
...

  {
    ...
    if (sample->timestamp_us <= ctx->last_ts_us) {  // 检查时间戳是否单调递增
      ctx->quality_flags |= IMPACT_DISP_QF_TS_NON_MONOTONIC;  // 设置质量标志
      delta_us = impact_disp_nominal_dt_us(&ctx->cfg);  // 使用名义时间间隔
      if (delta_us == 0u) {
        delta_us = 1u;  // 避免除零错误
      }
    } else {
    ...
    if (delta_us > ((uint32_t)ctx->cfg.max_dt_ms * 1000u)) {  // 检查时间差是否过大
      ctx->quality_flags |= IMPACT_DISP_QF_DT_GAP;  // 设置质量标志
    }
  ...
  if ((ctx->last_ts_us - ctx->first_ts_us) >  // 检查是否超过最大事件持续时间
    ...
    ctx->state = IMPACT_DISP_STATE_FINISHED;  // 设置状态为完成
  }
...
impact_disp_status_t impact_disp_end_event(impact_disp_ctx_t *ctx,
  ...
  if (ctx == NULL || out == NULL) {
    return IMPACT_DISP_ERR_ARG;  // 如果上下文或输出为空，返回参数错误
  }
  if (ctx->state != IMPACT_DISP_STATE_COLLECTING &&  // 检查状态是否正确
      ctx->state != IMPACT_DISP_STATE_FINISHED) {
    return IMPACT_DISP_ERR_STATE;
  }
  if (ctx->sample_count < 2u) {  // 检查采样数量是否足够
    ctx->quality_flags |= IMPACT_DISP_QF_LOW_SAMPLE_COUNT;  // 设置质量标志
  ...
  if ((ctx->last_ts_us - ctx->first_ts_us) <  // 检查事件是否持续时间过短
    ...
    ctx->quality_flags |= IMPACT_DISP_QF_DURATION_TOO_SHORT;  // 设置质量标志
  }

  if (ctx->release_count < ctx->cfg.release_count_min) {  // 检查是否达到最小释放次数
    ctx->quality_flags |= IMPACT_DISP_QF_NO_RELEASE;      // 设置质量标志
  }
  ...
  if (ctx->baseline_set != 0u && ctx->tail_count > 0u) {  // 如果设置了基准且有尾部数据
    ...
    rot_err_mg = impact_disp_norm3(dgx, dgy, dgz);  // 计算旋转误差

    if (rot_err_mg > impact_disp_rotation_limit_mg(ctx->cfg.rotation_limit_deg)) {
      ctx->quality_flags |= IMPACT_DISP_QF_ROTATION_HIGH;  // 如果旋转误差过大，设置质量标志
    }
  }
  ...
  out->disp_mm = impact_disp_norm3(out->dx_mm, out->dy_mm, out->dz_mm);  // 总位移(毫米)
  ...
  out->confidence = impact_disp_clamp_u8(impact_disp_confidence_from_flags(  // 置信度
...
impact_disp_status_t impact_disp_process_event(impact_disp_ctx_t *ctx,
  ...
  impact_disp_status_t st; // 状态返回值

  if (ctx == NULL || samples == NULL || out == NULL) {
    return IMPACT_DISP_ERR_ARG;  // 如果参数为空，返回参数错误
  }
...
impact_disp_state_t impact_disp_get_state(const impact_disp_ctx_t *ctx) {
  if (ctx == NULL) {
    return IMPACT_DISP_STATE_ERROR;  // 如果上下文为空，返回错误状态
  }
