...
- `../test/build_and_run.bat`：单场景运行与 `all` 一键回归入口
- `../test/run_all_scenarios.ps1`：全场景（`0..13`）自动回归与汇总报告生成
...
- 全场景自动回归报告默认输出到 `../test/logs/regression_summary_*.md`。
- 建议在每次协议改动后至少执行一次 `build_and_run.bat all`，并保留本轮报告文件用于审查追踪。
